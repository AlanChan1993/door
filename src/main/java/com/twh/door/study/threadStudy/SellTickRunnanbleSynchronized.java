...
public class SellTickRunnanbleSynchronized implements Runnable {
    // 定义变量模拟要卖的100张票
    ...
    public void run() {
        while (ticket > 0) { // "a"首先是String对象. 且常量字符串只会在方法区常量池中存储一份.
            ...
            synchronized (SellTickRunnanbleSynchronized.class) { // 类名.class就是获取这个类在方法区中加载字节文件之后形成字节码文件对象.一个类字节文件只会在方法区中加载一次.这个类字节文件对象只有一个.它也能充当锁对象
    ...
    public static void main(String[] args) {
        // 卖票任务
        SellTickRunnanbleSynchronized st = new SellTickRunnanbleSynchronized();
        // 创建3个线程对象,模拟三个窗口
